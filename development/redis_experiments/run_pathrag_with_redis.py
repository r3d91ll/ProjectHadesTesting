...
This script runs the PathRAG embedding process with Redis integration.
It sets up Redis as a high-performance cache for vector operations,
loads documents into Redis, runs the embedding process, and exports
the results to disk for permanent storage.

Usage:
    python run_pathrag_with_redis.py --source-dir /path/to/source --output-dir /path/to/output [--gpu]
"""
...
def main():
    ...
    try:
        ...
        logger.info("Running PathRAG embedding process with Redis integration...")
        
        # Set environment variables for Redis
        env = os.environ.copy()
        env["PATHRAG_REDIS_ENABLED"] = "true"
        env["PATHRAG_REDIS_HOST"] = args.redis_host
        env["PATHRAG_REDIS_PORT"] = str(args.redis_port)
        env["PATHRAG_REDIS_DB"] = str(args.redis_db)
        env["PATHRAG_REDIS_PREFIX"] = args.redis_prefix
        env["PATHRAG_REDIS_TTL"] = str(86400 * 7)  # 7 days
        if args.redis_password:
            env["PATHRAG_REDIS_PASSWORD"] = args.redis_password
        
        # Run the PathRAG embedding process
        rag_builder_dir = os.path.join(project_root, "rag-dataset-builder")
        
        # Build the command to use the existing config file
        ...
        logger.error(f"Error in PathRAG with Redis integration: {e}")
        import traceback
